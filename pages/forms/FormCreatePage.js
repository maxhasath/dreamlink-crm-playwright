...
class FormCreatePage extends BasePage {
  ...
  async expectLoaded() {
    await expect(this.page).toHaveURL(/\/forms\/new/, {
      timeout: TIMEOUTS.navigation,
    });
    await expect(this.surveyTitleInput).toBeVisible({
      timeout: TIMEOUTS.default,
    });
  }

  async addQuestion() {
    await this.addQuestionBtn
      .first()
  ...
  async fillSurveyTitle(title) {
    await this.surveyTitleInput.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
  ...
  async saveAndPublish() {
    await this.saveAndPublishBtn.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
  ...
  async expectSuccessToast(formName) {
    ...
    await expect(this.successToastTitle).toContainText(
      `Form "${formName}" created and published successfully.`,
      { timeout: TIMEOUTS.default },
    );
  }
}
