...
class GeofencePage extends BasePage {
  constructor(page) {
    ...
    this.geofenceInvalidMessage = page.locator(SELECTORS.geofenceInvalidSearchMessage);
  ...
  async expectCountryDataExists() {
    ...
    await expect(this.geofenceListRow.first()).toBeVisible({ timeout: TIMEOUTS.default });
  }

  async expectFoDataExists() {
    await this.loadingIndicator.waitFor({ state: 'hidden', timeout: TIMEOUTS.default });
    await this.geofenceFoSelect.waitFor({ state: 'visible', timeout: TIMEOUTS.default });
  ...
  async clickOrgHierarchy() {
    ...
    await this.geofenceOrgSummary.waitFor({ state: 'visible', timeout: TIMEOUTS.default });
  }

  async searchGeofenceByName(keyword) {
    await this.geofenceSearchInput.waitFor({ state: 'visible', timeout: TIMEOUTS.default });
    await this.geofenceSearchInput.fill(keyword);
    await expect(this.geofenceListRow.first()).toBeVisible({ timeout: TIMEOUTS.default });
  }

  async clickFocusOnMap() {
    await this.focusOnMapBtn.waitFor({ state: 'visible', timeout: TIMEOUTS.default });
  ...
  async expectResultCount(expectedCount) {
    await expect(this.geofenceResultsList.locator('li'))
      .toHaveCount(expectedCount, { timeout: TIMEOUTS.default });
  }

  async expectMapVisible() {
    await expect(this.geofenceMapContainer).toBeVisible({ timeout: TIMEOUTS.default });
  }

  async expectInvalidSearchMessage() {
    await expect(this.geofenceInvalidMessage).toBeVisible({ timeout: TIMEOUTS.default });
  }
...
async expectAssignedUsersNotEmpty() {
  const usersSection = this.configureUsersBtn
    .locator('xpath=ancestor::section');
  await expect(usersSection.locator(SELECTORS.geofenceAssignedItemName))
    .not.toHaveCount(0, { timeout: TIMEOUTS.default });
}

async expectAssignedProductsNotEmpty() {
  const productsSection = this.configureProductsBtn
    .locator('xpath=ancestor::section');
