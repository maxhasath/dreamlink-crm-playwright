...
class GeofenceDetailPage extends BasePage {
  constructor(page) {
    ...
    this.geofenceDetailPageSubtitle = page
      .locator(SELECTORS.geofenceDetailPageSubtitle)
      .first();
    this.geofenceDetailPageAssignUser = page
      .locator(SELECTORS.geofenceDetailPageAssignUser)
      .nth(1);
    this.geofenceAssignUserModalTitle = page.getByText(
      "Configure User Assignments",
      { exact: true },
    );
    this.geofenceAssignUserCancelBtn = page
      .locator(SELECTORS.geofenceAssignUserCancelBtn)
      .nth(3);
    ...
    this.geofenceBranchFirstItem = page
      .locator(SELECTORS.geofenceBranchFirstItem)
      .first();
    this.geofenceBranchTab = page.locator(SELECTORS.geofenceBranchTab).nth(4);
    this.geofenceBranchTitle = page
      .locator(SELECTORS.geofenceBranchTitle)
      .first();
    this.geofenceDetailPageAssignProduct = page
      .locator(SELECTORS.geofenceDetailPageAssignUser)
      .nth(2);
    this.geofenceAssignProductModalTitle = page.getByText(
      "Configure Product Assignments",
      { exact: true },
    );
    this.geofenceAssignProductCancelBtn = page
      .locator(SELECTORS.geofenceAssignProductCancelBtn)
      .nth(3);
  }
  ...
  async expectGeofenceDetailPage() {
    await this.geofenceNavLink.waitFor({
      state: "visible",
    ...
    await this.geofenceDetailPageSubtitle.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
    await this.geofenceDetailPageAssignUser.click();
    await this.geofenceAssignUserModalTitle.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
  ...
  async expectGeofenceProductModal() {
    ...
    await this.geofenceBranchTitle.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
    await this.geofenceDetailPageAssignProduct.click();
    await this.geofenceAssignProductModalTitle.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
