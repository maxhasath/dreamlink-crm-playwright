...
class AddProductPage extends BasePage {
  constructor(page) {
    ...
    this.productDSCriteriaSetSelect = page
      .locator(".rows > div > .data-row > div:nth-child(4)")
      .first();
    this.productDSCriteriaSetForm = page.locator(
      "div:nth-child(7) > .section-body > .form-column",
    );
  ...
  async clickAddProduct() {
    await this.addProductBtn.waitFor({
      state: "visible",
  ...
  async fillProductDescription(description) {
    await this.productDescriptionInput.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
  ...
  async selectEligibilityCriteriaSet(criteriaSetName) {
    await this.productDSCriteriaSetSelect.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
  ...
  async clickSaveProduct() {
    await this.productSaveBtn.waitFor({
      state: "visible",
      timeout: TIMEOUTS.default,
    });
