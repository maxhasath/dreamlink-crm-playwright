...
class PreLeadDetailPage {
  constructor(page) {
    ...
    this.preLeadSavingGroupValue = page.locator(SELECTORS.preLeadSavingGroupValue);
  ...
  async expectSavingGroup(groupName) {
    await expect(this.preLeadSavingGroupValue).toBeVisible({ timeout: TIMEOUTS.default });
    await expect(this.preLeadSavingGroupValue).toContainText(groupName);
  }

  async expectGeneratedDateIsToday() {
    await expect(this.preLeadGeneratedDate).toBeVisible({ timeout: TIMEOUTS.default });
