...
test.describe("DreamLink CRM - Tasks", () => {
  ...
  test("TC-TK-01 | Tasks list page loads and displays at least one task record", async ({
    page,
  }) => {
  ...
  test("TC-TK-02 | Clicking the first task navigates to detail page with Number, Title, Reference, Stage and Status all populated", async ({
    page,
  }) => {
  ...
  test("TC-TK-03 | Search by task number returns exactly 1 result containing the searched keyword", async ({
    page,
  }) => {
