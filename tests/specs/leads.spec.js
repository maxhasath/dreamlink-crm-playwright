...
test.describe("DreamLink CRM - Leads", () => {
  ...
  test("TC-LD-01 | Leads list page loads and displays at least one lead record", async ({
    page,
  }) => {
    await leadsPage.expectLeadListNotEmpty();
  });

  test("TC-LD-02 | Clicking the first lead navigates to detail page with Number, Saving Group, Current Workflow Stage and Tasks list all populated", async ({
    page,
  }) => {
  ...
  test("TC-LD-03 | Search by lead number returns exactly 1 result containing the searched keyword", async ({
    page,
  }) => {
